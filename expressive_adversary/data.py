"""FFL(T, p) data primitives.

COPIED VERBATIM from `flip_flop/data.py` (Liu et al. 2023, Section 3.1) so this
package is self-contained and cannot perturb the running flip_flop tree. Do not
edit — keep byte-faithful to the original; the controller's correctness rests on
`enforce_read_determinism` being exactly the project's centralized validity gate.
...
def write_read_gaps(tokens):
    ...
    instruction, measured in tokens. Pure bookkeeping on already-sampled
    tokens — zero model forward passes. Returns (mean_gap, p90_gap);
    (0.0, 0.0) if the batch has no reads.
    """
