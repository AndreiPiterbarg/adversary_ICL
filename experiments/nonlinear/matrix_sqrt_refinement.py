"""
Matrix Square Root Inversion — Nonlinear Refinement Experiment

Given SPD matrix A, predict x = A^{-1/2} b.  The model receives A (not A^{1/2}),
so one-step linear inversion is insufficient.  Context pairs show (b_i, A^{-1/2} b_i).

Hypothesis test: compare model corrections against a preconditioned gradient step
    delta = -A^{-1/2} (A^{1/2} x_k - b)
computed via eigendecomposition.  This is the Newton step for the system
    A^{1/2} x = b.

Uses lower condition number range (kappa 1-20) to keep the problem tractable.

Usage:
    python experiments/nonlinear/matrix_sqrt_refinement.py --device cuda
"""
...
from data.matrix_sqrt_sampler import make_batch_matrix_sqrt, gradient_correction
...
def train(device_str, training_steps=80000):
    device = torch.device(device_str)
    d, B, K = 4, 64, 5

    print("=" * 60)
    print("MATRIX SQUARE ROOT INVERSION — Nonlinear Refinement")
    print("=" * 60)
    print(f"  d={d}, B={B}, K={K}, steps={training_steps}, kappa=[1, 20]")
    ...
    for step in range(training_steps):
        matrix, ctx_in, ctx_out, q_in, q_target = make_batch_matrix_sqrt(
            B, K, d, device, kappa_min=1.0, kappa_max=20.0)
...
# ============================================================
# Evaluation with gradient hypothesis testing
# ============================================================
...
def evaluate(model, trainer, device, max_iters=20, test_batches=30, B=64):
    ...
    for damping in [1.0, 0.5]:
        ...
        for _ in range(test_batches):
            with torch.no_grad():
                matrix, ctx_in, ctx_out, q_in, q_target = \
                    make_batch_matrix_sqrt(B, K, d, device,
                ...
                for i in range(1, max_iters):
                    # Analytical gradient correction
                    grad_delta = gradient_correction(
                        matrix, q_in, x_current)
        ...
        results[tag] = {
            ...
            "grad_cosine_per_iter": [None] + cosine_means,
            "best_iter": best_iter,
...
def print_results(results):
    for mode in ["undamped", "damped_0.5"]:
        ...
        print(f"\n  MATRIX SQRT{tag}:")
        print(f"    {'iter':<6} {'MSE':<14} {'Grad cos':<12}")
        print(f"    {'-'*35}")
        for i, mse in enumerate(r["mse_per_iter"]):
            marker = " <-- best" if i == r["best_iter"] else ""
            cos_str = (f"{r['grad_cosine_per_iter'][i]:.4f}"
                       if r['grad_cosine_per_iter'][i] is not None
                       else "  --")
...
def main():
    parser = argparse.ArgumentParser(
        description="Matrix square root inversion — nonlinear refinement")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--training_steps", type=int, default=80000)
    ...
    output = {
        "task": "MATRIX_SQRT_INVERSION",
        "desc": "A^{-1/2} b prediction, low kappa SPD",
        "train_time": train_time,
        "training_steps": args.training_steps,
        "results": results,
    }
    out_path = output_dir / "matrix_sqrt_results.json"
