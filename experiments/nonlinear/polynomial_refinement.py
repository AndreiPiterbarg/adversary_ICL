"""
Polynomial Root Finding — Nonlinear Refinement Experiment

System of d=4 independent cubic polynomials.  Each polynomial has 3 real roots;
context examples demonstrate convergence from starting points to one specific root.
The model learns Newton's method for root finding.

Hypothesis test: compare model corrections against analytical Newton corrections
    delta_j = -p_j(x_k[j]) / p_j'(x_k[j])
at each refinement step and measure cosine similarity.

Usage:
    python experiments/nonlinear/polynomial_refinement.py --device cuda
"""
...
from data.polynomial_sampler import make_batch_polynomial, newton_correction


# ============================================================
# Training infrastructure (mirrors train_refinement.py)
# ============================================================
...
def train(device_str, training_steps=80000):
    ...
    print("=" * 60)
    print("POLYNOMIAL ROOT FINDING — Nonlinear Refinement")
    print("=" * 60)
    ...
    for step in range(training_steps):
        matrix, ctx_in, ctx_out, q_in, q_target = make_batch_polynomial(
            B, K, d, device)
...
# ============================================================
# Evaluation with Newton hypothesis testing
# ============================================================
...
def evaluate(model, trainer, device, max_iters=20, test_batches=30, B=64):
    ...
    for damping in [1.0, 0.5]:
        ...
        for _ in range(test_batches):
            with torch.no_grad():
                matrix, ctx_in, ctx_out, q_in, q_target = \
                    make_batch_polynomial(B, K, d, device)
                ...
                for i in range(1, max_iters):
                    # Analytical Newton correction at current point
                    newton_delta = newton_correction(matrix, x_current)
                    ...
                    # Cosine similarity between model and Newton corrections
                    cos_sim = F.cosine_similarity(
                        model_delta.reshape(-1, d),
                        newton_delta.reshape(-1, d), dim=-1)
        ...
        results[tag] = {
            ...
            "newton_cosine_per_iter": [None] + cosine_means,
            "best_iter": best_iter,
...
def print_results(results):
    for mode in ["undamped", "damped_0.5"]:
        ...
        print(f"\n  POLYNOMIAL{tag}:")
        print(f"    {'iter':<6} {'MSE':<14} {'Newton cos':<12}")
        print(f"    {'-'*35}")
        for i, mse in enumerate(r["mse_per_iter"]):
            marker = " <-- best" if i == r["best_iter"] else ""
            cos_str = (f"{r['newton_cosine_per_iter'][i]:.4f}"
                       if r['newton_cosine_per_iter'][i] is not None
                       else "  --")
...
def main():
    parser = argparse.ArgumentParser(
        description="Polynomial root finding — nonlinear refinement")
    ...
    output = {
        "task": "POLYNOMIAL_ROOT_FINDING",
        "desc": "d=4 independent cubics, Newton root finding",
        "train_time": train_time,
        "training_steps": args.training_steps,
        "results": results,
    }
    out_path = output_dir / "polynomial_results.json"
